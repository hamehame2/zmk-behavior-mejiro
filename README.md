...
本家のメジロ式はNKROのキーボードかつploverが必要と少し実装の敷居が高いことがあげられます。

https://github.com/JEEBIS27/Plover_Mejiro

plover以外のものとしてAutoHotKey版を私が公開しましたが、NKROが必要なことに変わりはありません。また若干参照元にした版が古いままです。

https://github.com/hamehame2/AHK_Mejiro

またQMK版を自作キーボードに導入する手順はploverより込み入っており、セットアップ済みのMejiro31は人気で手にしづらいなどがあること、またQMKの無線は実現ができないことがありました。

https://github.com/JEEBIS27/qmk_firmware/tree/master/keyboards/jeebis/mejiro31

今回公開しているZMK版はプロジェクト(モジュール)取り込み式ですのでキーボードを問わず比較的簡単に導入できるのが利点です。

zmk関係なく物理配列として一般的なキーボード配列ではcvb位置に左ntkを配置した場合親指キーが横一直線に3つ並びとなります。

　この場合、隣同士のcv(nt) vb(tk)は親指で打てるものの cb(nk)とcvb(ntk)の打鍵が厳しいため救済措置として-ntk、-nk、ntk-、nk-を押したことにするキーを本zmk版では追加しています。
...
zmkのエコシステムのため、プロジェクトurlや名前の入れ替えでキーマップはそのままでzmkメジロ式v2からzmk新下駄にそのまま変更できる互換性があります。zmk新下駄は下記にて公開中しています。

https://github.com/hamehame2/zmk-naginata

ただしeswai様の本家zmk薙刀式にする場合はng SQTをユーザーのキーマップから削除するか、フォークした薙刀式にng SQT定義を追加してください。

最初からng SQTを使用しなければ3つのプロジェクトは同じキーマップで互換性があります。

　メジロ式にする場合親指ntk配置の関係上、キーマップを変更しないと厳しいと思いますのでブランチでキーマップを変更したのを持つ運用に落ち着くとは思います。
...
最新版は２つファイル名違いで（src/behaviorsの中に　behavior_naginata.c、ver名や日付など記載したの）を残して最新と保管の運用ができたらと思います。
