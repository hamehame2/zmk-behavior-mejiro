...
// ZMK does not provide QMK-style send_string(); provide a minimal stub.
static void send_string(const char *s) {
    /* Minimal sender for debugging: enough for Mejiro's romaji output.
     * (ASCII letters + a few punctuation handled by send_ascii_letters_only)
     */
...
static void send_ascii_letters_only(const char *s) {
    for (const char *p = s; *p; p++) {
        ...
        if (c == '#' || c == '*' || c == '-') {
            tap_key(keycode_from_ascii_letter(c));
            continue;
        }
...
/* Safer alternatives to strcpy/strcat (BSD strl* style). */
static size_t mj_strlcpy(char *dst, const char *src, size_t dst_size) {
    if (!dst || dst_size == 0) {
        return 0;
    }
    if (!src) {
    ...
    dst[i] = '\0';
    /* Return full length of src (like strlcpy). */
    while (src[i] != '\0') {
        i++;
    }
    return i;
...
static size_t mj_strlcat(char *dst, const char *src, size_t dst_size) {
    if (!dst || dst_size == 0) {
        return 0;
    }
    size_t dlen = strnlen(dst, dst_size);
    ...
    size_t i = 0;
    for (; dlen + i + 1 < dst_size && src[i] != '\0'; i++) {
        dst[dlen + i] = src[i];
    }
    dst[dlen + i] = '\0';
    /* Return dlen + full length of src (like strlcat). */
    while (src[i] != '\0') {
        i++;
    }
    return dlen + i;
...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    /* SAFE MODE: bypass transform/output to isolate hangs/resets. */
#if MEJIRO_SAFE_MODE
    /* Safe mode: do not translate. Output the raw stroke id (ASCII) so we can
     * confirm the behavior is firing without crashing.
     */
    send_string(mejiro_id);
