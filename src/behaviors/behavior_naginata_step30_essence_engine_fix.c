...
static const verb_entry_t *find_registered_verb(const char *left_conso, const char *left_vowel,
...
static bool mejiro_conjugate_exact_dict_verb(const char *left_conso, const char *left_vowel,
    ...
    const verb_entry_t *verb = find_registered_verb(left_conso, left_vowel, right_conso, right_vowel);
    if (verb == NULL) {
        return false;
    }
...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    if (has_asterisk) {
        ...
        if (strcmp(r_conso, "TN") == 0 && strlen(r_vowel) == 0 &&
            find_registered_verb(l_conso, l_vowel, r_conso, r_vowel) == NULL) {
