...
static bool mejiro_conjugate_inferred_verb(const char *left_particle, const char *right_particle,
    ...
    /* 右側が空なら未定義サ変として作る */
    if (strlen(right_kana) == 0 && strlen(left_kana) > 0) {
        strncat(out, left_kana, out_size - strlen(out) - 1);
        strncat(out, sahen_conjugate[conj_form], out_size - strlen(out) - 1);
        char *shizu_pos = strstr(out, "しず");
        if (shizu_pos) {
    ...
    if (strlen(right_conso) > 0 && strlen(right_vowel) == 0 && strlen(right_kana) > 0) {
        ...
        if (gyou == 'k' || gyou == 'g' || gyou == 's' || gyou == 't' ||
    ...
    if (strcmp(right_vowel, "I") == 0) {
        char gyou = strlen(right_kana) > 0 ? kana_to_gyou(right_kana) : 'w';
        if (gyou == '\0') { gyou = 'w'; }
    ...
    /* 最後の fallback: 右かなを語幹末尾としてラ行五段扱い */
    if ((strlen(right_conso) > 0 || strlen(right_vowel) > 0) && strlen(right_kana) > 0) {
        strncat(out, left_kana, out_size - strlen(out) - 1);
