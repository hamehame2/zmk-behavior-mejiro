...
/* Forward declarations (avoid implicit declarations with -Werror=implicit-int) */
static void clear_nginput_safely(void);
/* HID helpers (avoid implicit declarations) */
int zmk_hid_keyboard_press(uint8_t keycode);
int zmk_hid_keyboard_release(uint8_t keycode);
...
#include <stddef.h>
/* =========================================================
 * Safety: never block the event thread on broken state
 *  - nginput.size can be corrupted (OOB/NULL/logic bug). A raw while(remove at 0)
 *    can become extremely slow (O(n^2)) or infinite, leading to ~4s WDT/fatal.
 * ========================================================= */
static void clear_nginput_safely(void) {
    /* Hard cap to avoid long/infinite loops if nginput.size is corrupted. */
    const size_t hard_cap = 64;

    if (nginput.size > hard_cap) {
        /* Broken state suspected: force reset to keep firmware alive. */
        nginput.size = 0;
        return;
    }
