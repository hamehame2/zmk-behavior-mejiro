...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    const bool has_hash = mejiro_contains_hash_local(mejiro_id);
    char hashless[64] = {0};
    const char *transform_stroke = mejiro_id;

    if (has_hash) {
        mejiro_remove_hash_local(mejiro_id, hashless, sizeof(hashless));
        if (hashless[0] != '\0') {
            transform_stroke = hashless;
        }
    }

    mejiro_result_t_zmk result = mejiro_transform_zmk(transform_stroke);
    if (!result.success) {
