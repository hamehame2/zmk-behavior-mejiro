...
abbreviation_result_t mejiro_user_abbreviation(const char *stroke) {
    ...
    for (size_t i = 0; user_abbreviations[i].stroke != NULL; i++) {
        if (strcmp(stroke, user_abbreviations[i].stroke) == 0) {
            mj_strlcpy(result.output, user_abbreviations[i].output, sizeof(result.output));
            result.success = true;
            return result;
        }
    }
...
abbreviation_result_t mejiro_abstract_abbreviation(const char *stroke) {
    ...
    for (size_t i = 0; abstract_abbreviations[i].stroke != NULL; i++) {
        if (strcmp(stroke, abstract_abbreviations[i].stroke) == 0) {
            mj_strlcpy(result.output, abstract_abbreviations[i].output, sizeof(result.output));
            result.success = true;
            return result;
        }
    }
    ...
    if (hyphen != NULL) {
        ...
        /* bounded copy of left part */
        {
            size_t n = left_len;
            if (n > (sizeof(left_part) - 1)) {
                n = sizeof(left_part) - 1;
            }
            memcpy(left_part, stroke, n);
            left_part[n] = '\0';
        }
        mj_strlcpy(right_part, hyphen + 1, sizeof(right_part));
const char *left_output = NULL;
        ...
        if (left_output != NULL && right_output != NULL) {
            mj_strlcpy(result.output, left_output, sizeof(result.output));
            mj_strlcat(result.output, right_output, sizeof(result.output));
result.success = true;
        }
    }
...
static void get_conjugation_info(const char *left_particle, const char *right_particle,
    ...
    for (int i = 0; auxiliary_exception[i].conj_form != -1; i++) {
        if (strcmp(pattern, auxiliary_exception[i].pattern) == 0) {
            *conj_form = auxiliary_exception[i].conj_form;
            mj_strlcpy(suffix, auxiliary_exception[i].suffix, sizeof(suffix));
            return;
        }
    }
    ...
    for (int i = 0; right_auxiliary[i].particle != NULL; i++) {
        if (strcmp(right_particle, right_auxiliary[i].particle) == 0) {
            *conj_form = right_auxiliary[i].conj_form;
            mj_strlcpy(suffix, right_auxiliary[i].suffix, sizeof(suffix));
            return;
        }
    }
    ...
    *conj_form = CONJ_JISHO;
    ...
    static void append_left_auxiliary(char *out, const left_auxiliary_info_t *left_aux, const right_auxiliary_t *right_aux) {
        ...
        mj_strlcat(out, left_aux->stem, sizeof(out));
int aux_conj = (right_aux != NULL) ? right_aux->conj_form : CONJ_JISHO;
        if (left_aux->verb_type == 1) {
            int idx_aux = gyou_to_index(left_aux->gyou);
            mj_strlcat(out, godan_conjugate[idx_aux][aux_conj], sizeof(out));
} else if (left_aux->verb_type == 2) {
            int idx_aux = gyou_to_index(left_aux->gyou);
            mj_strlcat(out, kami_conjugate[idx_aux][aux_conj], sizeof(out));
} else if (left_aux->verb_type == 3) {
            int idx_aux = gyou_to_index_simo(left_aux->gyou);
            mj_strlcat(out, simo_conjugate[idx_aux][aux_conj], sizeof(out));
}

        if (right_aux != NULL) {
            mj_strlcat(out, right_aux->suffix, sizeof(out));
}
    }
...
verb_result_t mejiro_verb_conjugate(const char *left_conso, const char *left_vowel,
    ...
    if (strcmp(right_conso, "TN") == 0 && strlen(right_vowel) == 0) {
        char base[128] = {0};
        mj_strlcat(base, left_kana, sizeof(base));
// 左側の助詞追加音は含める（例: TAn-TN* → たんです）
        mj_strlcat(base, get_particle_extra(left_particle), sizeof(base));
        // 右側の助詞追加音は含めない（例: -TNk* → でしょう）

        mj_strlcpy(result.output, base, sizeof(result.output));
        const char *desu_form = get_desu_conjugate(right_particle);
        if (desu_form != NULL) {
            mj_strlcat(result.output, desu_form, sizeof(result.output));
result.success = true;
            return result;
        }
    }
    ...
    if (verb != NULL) {
        if (verb->type == VERB_TYPE_SPECIAL) {
            if (strcmp(stroke, "I-K") == 0) {
                mj_strlcpy(result.output, iku_conjugate[conj_form], sizeof(result.output));
            } else if (strcmp(stroke, "A-") == 0) {
                mj_strlcpy(result.output, aru_conjugate[conj_form], sizeof(result.output));
            } else {
                return result;
            }
        } else {
            mj_strlcpy(result.output, verb->stem, sizeof(result.output));
            if (verb->type == VERB_TYPE_GODAN) {
                int idx = gyou_to_index(verb->gyou, sizeof(result.output));
                mj_strlcat(result.output, godan_conjugate[idx][conj_form], sizeof(result.output));
} else if (verb->type == VERB_TYPE_KAMI) {
                int idx = gyou_to_index(verb->gyou);
                mj_strlcat(result.output, kami_conjugate[idx][conj_form], sizeof(result.output));
} else if (verb->type == VERB_TYPE_SIMO) {
                int idx = gyou_to_index_simo(verb->gyou);
                mj_strlcat(result.output, simo_conjugate[idx][conj_form], sizeof(result.output));
} else if (verb->type == VERB_TYPE_KAHEN) {
                mj_strlcat(result.output, kahen_conjugate[conj_form], sizeof(result.output));
}
        }
        ...
        if (left_aux != NULL) {
            ...
            mj_strlcat(result.output, suffix, sizeof(result.output));
}
    ...
    if (strlen(right_kana) == 0) {
        if (strlen(left_kana) > 0) {
            mj_strlcpy(result.output, left_kana, sizeof(result.output));
        } else {
            result.output[0] = '\0';
        }
        mj_strlcat(result.output, sahen_conjugate[conj_form], sizeof(result.output));
char *shizu_pos = strstr(result.output, "しず");
    ...
    if (strlen(right_conso) > 0 && strlen(right_vowel) == 0 && strlen(right_kana) > 0) {
        ...
        if (gyou != '\0' && (gyou == 'k' || gyou == 'g' || gyou == 's' || gyou == 't' ||
            ...
            int idx = gyou_to_index(gyou, sizeof(result.output));
            mj_strlcat(result.output, godan_conjugate[idx][conj_form], sizeof(result.output));
if (left_aux != NULL) {
                append_left_auxiliary(result.output, left_aux, right_aux);
            } else {
...
if (left_aux != NULL) {
    ...
    if (strcmp(right_vowel, "IA") == 0) {
        ...
        if (gyou == 'k' || gyou == 'g' || gyou == 's' || gyou == 'z' ||
            ...
            int idx = gyou_to_index_simo(gyou, sizeof(result.output));
            mj_strlcat(result.output, simo_conjugate[idx][conj_form], sizeof(result.output));
if (left_aux != NULL) {
                append_left_auxiliary(result.output, left_aux, right_aux);
            } else {
    ...
    if (strlen(right_conso) > 0 || strlen(right_vowel) > 0) {
        if (strlen(left_kana) > 0) {
            mj_strlcpy(result.output, left_kana, sizeof(result.output));
            mj_strlcat(result.output, right_kana, sizeof(result.output));
} else {
            mj_strlcpy(result.output, right_kana, sizeof(result.output));
        }
        int idx = gyou_to_index('r', sizeof(result.output));
        mj_strlcat(result.output, godan_conjugate[idx][conj_form], sizeof(result.output));
...
static void send_string(const char *s) {
    ...
    /* IMPORTANT (debug stability):
     *  - Do NOT use k_work_delayable based TX loop here.
     *  - We send synchronously from the behavior context to avoid delayed-work re-entrancy,
     *    which was causing a 3-4s crash on some builds.
     *
     * safe_mode meanings (kept compatible with your previous tests):
     *   0: normal behavior (still ASCII-only sender for stability; non-ASCII is ignored)
     *   1: always send 'A' only
     *   2: do not send anything (but keep the firmware alive)
     */

    if (g_mejiro_safe_mode == 1) {
        send_ascii_letters_only("A");
        return;
    }
    if (g_mejiro_safe_mode == 2) {
        return;
    }

    /* Normal path: for now, keep it ASCII letters + space only.
     * This lets us confirm the crash is NOT in delayed work.
     * Once stable, we can re-enable full keycode emission gradually.
     */
    send_ascii_letters_only(s);
}
...
/* =========================================================
 *  Safe string helpers (strlcpy/strlcat replacement)
 *  - Zephyr/picolibc environment may not provide BSD strl*.
 *  - These avoid buffer overruns that can cause delayed HardFaults.
 * ========================================================= */
static size_t mj_strlcpy(char *dst, const char *src, size_t siz) {
...
static size_t mj_strlcat(char *dst, const char *src, size_t siz) {
    size_t dstlen = dst ? strnlen(dst, siz) : 0;
    size_t srclen = src ? strlen(src) : 0;
    ...
    if (dstlen >= siz) {
        /* dst is not NUL-terminated within siz */
        return siz + srclen;
    }

    size_t space = siz - dstlen - 1; /* space for bytes excluding NUL */
...
static void convert_to_kana(const char *conso, const char *vowel, const char *particle_str,
    ...
    // 例外的なかなのマッピングをチェック
    char conso_vowel[32];
    mj_strlcpy(conso_vowel, conso, sizeof(conso_vowel));
    mj_strlcat(conso_vowel, vowel, sizeof(conso_vowel));;
    ...
    if (strlen(conso) > 0 && strlen(vowel) == 0 && strcmp(conso, "STN") == 0) {
        if (include_extra_sound) {
            ...
            mj_strlcpy(out, extra, sizeof(out));
        } else {
            out[0] = '\0';
        }
        return;
    ...
    if (exception_kana != NULL) {
        // 例外かなを使用
        mj_strlcpy(out, exception_kana, sizeof(out));
        if (include_extra_sound) {
            const char *extra = get_second_sound(particle_str);
            mj_strlcat(out, extra, sizeof(out));
}
    } else {
        // 複雑二重母音をチェック（母音+助詞）
        char vowel_particle[32];
        mj_strlcpy(vowel_particle, vowel, sizeof(vowel_particle));
        mj_strlcat(vowel_particle, particle_str, sizeof(vowel_particle));;
        ...
        mj_strlcpy(out, base_kana, sizeof(out));
        mj_strlcat(out, suffix, sizeof(out));
mj_strlcat(out, extra_sound, sizeof(out));
}
}
...
static void transform_joshi(const char *left_stroke, const char *right_stroke, char *out) {
    ...
    if (strstr(right_stroke, "n") != NULL) {
        ...
        *q = '\0';
        ...
        mj_strlcpy(right_tk, right_stroke, sizeof(right_tk));
    }
    ...
    if (strcmp(left_stroke, "n") == 0) {
        mj_strlcpy(out, r_particle[r_idx], sizeof(out));
        mj_strlcat(out, "、", sizeof(out));
        ...
        mj_strlcpy(out, "の", sizeof(out));
        mj_strlcat(out, l_particle[l_idx], sizeof(out));
if (has_comma) mj_strlcat(out, "、", sizeof(out));
} else {
        mj_strlcpy(out, l_particle[l_idx], sizeof(out));
        mj_strlcat(out, r_particle[r_idx], sizeof(out));
if (has_comma) mj_strlcat(out, "、", sizeof(out));
}
}
...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    if (has_asterisk) {
        ...
        if (abstract_abbr.success) {
            ...
            mj_strlcpy(kana_output, abstract_abbr.output, sizeof(kana_output));
            ...
            if (strlen(l_particle_str) > 0 || strlen(r_particle_str) > 0) {
                ...
                if (strcmp(particle_pattern, "n-") == 0) {
                    mj_strlcat(kana_output, "である", sizeof(kana_output));;
                } else if (strcmp(particle_pattern, "-n") == 0) {
                    mj_strlcat(kana_output, "だ", sizeof(kana_output));;
                } else if (strcmp(particle_pattern, "n-n") == 0) {
                    mj_strlcat(kana_output, "だった", sizeof(kana_output));;
                } else if (strcmp(particle_pattern, "-ntk") == 0) {
                    mj_strlcat(kana_output, "です", sizeof(kana_output));;
                } else if (strcmp(particle_pattern, "n-ntk") == 0) {
                    mj_strlcat(kana_output, "でした", sizeof(kana_output));;
                } else {
                    // その他の助詞は通常通り処理
                    char joshi_output[128] = {0};
                    transform_joshi(l_particle_str, r_particle_str, joshi_output);
                    mj_strlcat(kana_output, joshi_output, sizeof(kana_output));;
                }
                result.kana_length = utf8_char_count(kana_output);
            }
        ...
        if (verb_result.success) {
            ...
            mj_strlcpy(kana_output, verb_result.output, sizeof(kana_output));
            result.kana_length = utf8_char_count(kana_output);
            kana_to_roma_zmk(kana_output, result.output, sizeof(result.output));
            result.success = true;
            return result;
    ...
    if (strlen(l_vowel) == 0 && strlen(l_conso) > 0 && strcmp(l_conso, "STN") != 0) {
        mj_strlcpy(l_vowel, last_vowel_stroke, sizeof(l_vowel));
    }
    // 右側の母音が空で、右側に子音がある場合、左の母音を使用（左も空なら前回の母音）
    if (strlen(r_vowel) == 0 && strlen(r_conso) > 0 && strcmp(r_conso, "STN") != 0) {
        if (strlen(l_vowel) > 0) {
            mj_strlcpy(r_vowel, l_vowel, sizeof(r_vowel));
        } else {
            mj_strlcpy(r_vowel, last_vowel_stroke, sizeof(r_vowel));
        }
    }

    // 使用した母音を保存（右が優先、なければ左、どちらもなければ保存しない）
    if (strlen(r_vowel) > 0) {
        mj_strlcpy(last_vowel_stroke, r_vowel, sizeof(last_vowel_stroke));
    } else if (strlen(l_vowel) > 0) {
        mj_strlcpy(last_vowel_stroke, l_vowel, sizeof(last_vowel_stroke));
    }
    
    // 「っ」の持ち越し判定（左側と右側を別々に判定）
    // ただし、複雑二重母音（母音+助詞）に該当する場合は「っ」を出力しないため、持ち越し対象から除外する
    char left_vowel_particle[32] = {0};
    mj_strlcpy(left_vowel_particle, l_vowel, sizeof(left_vowel_particle));
    mj_strlcat(left_vowel_particle, l_particle_str, sizeof(left_vowel_particle));;
    ...
    mj_strlcpy(right_vowel_particle, r_vowel, sizeof(right_vowel_particle));
    mj_strlcat(right_vowel_particle, r_particle_str, sizeof(right_vowel_particle));;
    ...
    if (is_particle_only) {
        ...
        // 前回持ち越しの「っ」を先頭に追加
        if (pending_tsu) {
            mj_strlcpy(result.output, "っ", sizeof(result.output));
            pending_tsu = false;
        } else {
            result.output[0] = '\0';
        }
        ...
        if (strlen(l_conso) > 0 || strlen(l_vowel) > 0) {
            ...
            mj_strlcat(result.output, left_kana, sizeof(result.output));
}
        ...
        if (is_left_plus_particle) {
            ...
            transform_joshi(l_particle_str, r_particle_str, joshi_output);
            mj_strlcat(result.output, joshi_output, sizeof(result.output));
}
        ...
        if (strlen(l_conso) == 0 && strlen(l_vowel) == 0 &&
            ...
            mj_strlcat(result.output, left_extra_sound, sizeof(result.output));
}
    ...
    if (is_ntk_n) {
        mj_strlcat(result.output, "ん", sizeof(result.output));
}
    ...
    if (strlen(result.output) > 0 && !is_right_only) {
        char kana_output[128];
        mj_strlcpy(kana_output, result.output, sizeof(kana_output));
        result.kana_length = utf8_char_count(kana_output);
        kana_to_roma_zmk(kana_output, result.output, sizeof(result.output));
        result.success = true;
    } else if (pending_tsu) {
...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    mj_strlcpy(g_mejiro_last_output, r.output, sizeof(g_mejiro_last_output));
    return;
