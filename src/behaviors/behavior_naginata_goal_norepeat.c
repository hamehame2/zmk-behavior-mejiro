 ...
 * --------------------------------------------------------- */
// ---------------------------------------------------------------------------
// Key tapping helper (press+release immediately)
//  - Avoids stuck keys / host key-repeat when release gets delayed or lost.
// ---------------------------------------------------------------------------

static void ng_send_kbd_report(void) {
#if NG_HAVE_ENDPOINTS
    (void)zmk_endpoints_send_report(0x07); // HID Keyboard
#endif
}
