...
// How long (ms) we keep a "tap" key pressed before releasing it.
// Must be short enough to avoid OS auto-repeat.
#define NG_HID_TAP_DELAY_MS 5
#endif

/* ----------------------------------------------------------------
 * Tap-key implementation
 *
 * Problem we are fixing:
 *  - If we "press" a key and schedule a single delayed work item to
 *    release keys later, and we *reschedule* that work on every tap,
 *    earlier key releases can be postponed (held down too long).
 *  - That looks exactly like: "one output, then it starts repeating".
 *
 * Solution:
 *  - Keep a small ring buffer of (keycode, due_time_ms).
 *  - A delayable work releases all entries whose due_time <= now.
 *  - The work is scheduled for the *earliest* due_time, never pushed later.
 * ---------------------------------------------------------------- */

#ifndef NG_TAPQ_CAP
#define NG_TAPQ_CAP 32
#endif
    ...
    for (size_t i = 0; i < ARRAY_SIZE(mejiro_commands_zmk); i++) {
        ...
        case MJ_CMD_UNDO: {
            ...
            for (uint16_t k = 0; k < n; k++) {
                ng_tap_key(ZMK_HID_USAGE(HID_USAGE_KEY, HID_USAGE_KEY_KEYBOARD_DELETE_BACKSPACE));
            }
            return true;
        }
        case MJ_CMD_KEY: {
            ng_tap_key(mejiro_commands_zmk[i].keycodee);
            return true;
        }
        case MJ_CMD_MOD_KEY: {
