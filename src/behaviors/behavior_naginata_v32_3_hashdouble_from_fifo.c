...
#define MEJIRO_ROMA_KEY_DELAY_MS 20
#endif
...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    const char *lookup = mejiro_id;

    if (has_hash) {
        mejiro_remove_hash(mejiro_id, transform_pattern, sizeof(transform_pattern));
        if (transform_pattern[0] != '\0') {
            lookup = transform_pattern;
        }
    }

    mejiro_result_t_zmk result = mejiro_transform_zmk(lookup);
    if (!result.success) {
