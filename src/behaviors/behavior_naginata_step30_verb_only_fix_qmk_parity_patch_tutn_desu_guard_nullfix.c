...
// 完全一致の登録動詞かどうかを確認
static bool is_registered_verb_stroke(const char *left_conso, const char *left_vowel,
...
static bool mejiro_conjugate_exact_dict_verb(const char *left_conso, const char *left_vowel,
    ...
    if (verb->type == VERB_TYPE_SPECIAL && strcmp(stroke, "A-") == 0 && strcmp(out, "ず") == 0) {
        strncpy(out, "あらず", out_size - 1);
        out[out_size - 1] = '\0';
    }

    if (verb->type == VERB_TYPE_SAHEN) {
...
verb_result_t mejiro_verb_conjugate(const char *left_conso, const char *left_vowel,
    ...
    if (strcmp(right_conso, "TN") == 0 && strlen(right_vowel) == 0 &&
        !is_registered_verb_stroke(left_conso, left_vowel, right_conso, right_vowel)) {
        char base[128] = {0};
    ...
    if (verb != NULL) {
        ...
        if (conj_form == CONJ_TE_TA &&
            verb->type == VERB_TYPE_GODAN &&
            (verb->gyou == 'g' || verb->gyou == 'n' || verb->gyou == 'b' || verb->gyou == 'm')) {
            apply_godan_te_ta_voicing(result.output, strlen(verb->stem));
        }

        if (verb->type == VERB_TYPE_SPECIAL && strcmp(stroke, "A-") == 0 && strcmp(result.output, "ず") == 0) {
...
static const mj_cmd_t mejiro_commands_zmk[] = {
    ...
    {"-SNI",   MJ_CMD_STRING,   0, 0, 0, "'\0'{#Left}"},
...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    if (has_asterisk) {
        ...
        if (strcmp(r_conso, "TN") == 0 && strlen(r_vowel) == 0 &&
            !is_registered_verb_stroke(l_conso, l_vowel, r_conso, r_vowel)) {
