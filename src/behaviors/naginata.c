...
/* ================================
 * Safety helpers (no behavior change)
 *  - Prevent buffer overruns in local scratch buffers.
 * ================================ */
...
static inline void safe_strcat(char *dst, size_t dst_sz, const char *src) {
    if (!dst || dst_sz == 0 || !src) {
        return;
    }
