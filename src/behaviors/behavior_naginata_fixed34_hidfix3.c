...
/* Optional: force HID report send between press/release to avoid "stuck key" repeat */
...
static inline void ng_send_report(void) {
#if NG_HAVE_ENDPOINTS
    (void)zmk_endpoints_send_report(0x07);
#endif
}

/* Optional: emergency "unstick" helper.
 * If any code path forgets to release a pressed key, the host will start key repeat.
 * These weak symbols let us call release-all/clear when available, without hard depending
 * on a specific ZMK version/API.
 */
extern void zmk_hid_keyboard_release_all(void) __attribute__((weak));
extern void zmk_hid_keyboard_clear(void) __attribute__((weak));

#ifndef NG_FORCE_RELEASE_ALL
#define NG_FORCE_RELEASE_ALL 1
#endif

static inline void ng_force_release_all(void) {
#if NG_FORCE_RELEASE_ALL
    if (zmk_hid_keyboard_release_all) {
        zmk_hid_keyboard_release_all();
        ng_send_report();
    } else if (zmk_hid_keyboard_clear) {
...
static inline void tap_key(uint32_t keycode) {
    /* Some hosts/boards will treat back-to-back press+release with no report flush
     * as a "stuck" key. Flush a report after each transition if endpoints are available.
     */
    zmk_hid_keyboard_press(keycode);
    ng_send_report();
    zmk_hid_keyboard_release(keycode);
    ng_send_report();
    ng_force_release_all();
}
