...
/* Optional endpoints API (report flush). */
#if __has_include(<zmk/endpoints.h>)
#include <zmk/endpoints.h>
#define NG_HAVE_ENDPOINTS 1
#else
#define NG_HAVE_ENDPOINTS 0
#endif
...
#include <zmk/hid.h>
#endif
#if __has_include(<zmk/hid_keyboard.h>)
...
#include <zmk/events/keycodee_state_changed.h>
...
// NOTE: When using `studio-rpc-usb-uart`, log output on the same transport can break the RPC framing.
// To avoid 4s "Studio RPC timeout" errors, disable debug logs while CONFIG_ZMK_STUDIO=y.
#if IS_ENABLED(CONFIG_ZMK_STUDIO)
#define MJ_MJ_LOG_DBG(...) do { } while (0)
#else
#define MJ_MJ_LOG_DBG(...) LOG_DBG(__VA_ARGS__)
#endif

// Backward-compat alias: some code paths (or older generated files) may still
// reference MJ_LOG_DBG(). Ensure it's always a macro to avoid implicit
// declaration and link errors.
#ifndef MJ_LOG_DBG
#define MJ_LOG_DBG(...) MJ_MJ_LOG_DBG(__VA_ARGS__)
#endif
static int64_t g_mejiro_last_timestamp = 0;
#define NONE 0
...
//    keycodee/shift issues during verification.
...
/* ---------------------------------------------------------
 * HID tap reliability fix (prevents OS key-repeat "stuck key")
 *  - Do NOT press+release in the same report.
 *  - We press immediately, then release from a delayed work item.
 *  - This keeps ng output working without the heavy-handed release_all().
 * --------------------------------------------------------- */
#ifndef NG_HID_TAP_DELAY_MS
#define NG_HID_TAP_DELAY_MS 5
#endif

#ifndef NG_TAPQ_CAP
#define NG_TAPQ_CAP 16
#endif

static uint32_t ng_tapq[NG_TAPQ_CAP];
static uint8_t ng_tapq_len;

static void ng_send_kbd_report(void) {
#if NG_HAVE_ENDPOINTS
    /* Keyboard usage page */
    (void)zmk_endpoints_send_report(0x07);
#endif
}

static void ng_tapq_flush_releases(struct k_work *work);

static K_WORK_DELAYABLE_DEFINE(ng_tapq_work, ng_tapq_flush_releases);

static void ng_tapq_flush_releases(struct k_work *work) {
    ARG_UNUSED(work);
    /* Release all queued keycodees, then flush once. */
    for (uint8_t i = 0; i < ng_tapq_len; i++) {
        zmk_hid_keyboard_release(ng_tapq[i]);
    }
    ng_tapq_len = 0;
    ng_send_kbd_report();
}

static void ng_tap_key(uint32_t keycodee) {
    /* Press now, flush, queue release later. */
    zmk_hid_keyboard_press(keycodee);
    ng_send_kbd_report();

    if (ng_tapq_len < NG_TAPQ_CAP) {
        ng_tapq[ng_tapq_len++] = keycodee;
        /* Reschedule: coalesce multiple taps into one release burst. */
        (void)k_work_reschedule(&ng_tapq_work, K_MSEC(NG_HID_TAP_DELAY_MS));
        return;
    }

    /* Queue overflow: force release to avoid stuck keys. */
    zmk_hid_keyboard_release(keycodee);
    ng_send_kbd_report();
}

static void tap_key(uint32_t keycodee) {
    ng_tap_key(keycodee);
}
...
static void send_string(const char *s) {
     ...
     * Once stable, we can re-enable full keycodee emission gradually.
     */
...
static inline void press_key(uint32_t keycodee) {
    zmk_hid_keyboard_press(keycodee);
}

static inline void release_key(uint32_t keycodee) {
    zmk_hid_keyboard_release(keycodee);
}

static inline void mod_tap(uint32_t mod_keycodee, uint32_t keycodee) {
    press_key(mod_keycodee);
    tap_key(keycodee);
    release_key(mod_keycodee);
}
 ...
 * Here we keep the same stroke strings and send the equivalent ZMK keycodees.
 * -------------------------------------------------------------------------- */
...
/* Mod keycodees (explicit HID usages, avoids relying on keycodee aliases) */
...
static const mj_cmd_t mejiro_commands_zmk[] = {
    ...
    /* QMK parity: keycodee commands that include '#' */
...
static bool handle_mejiro_command(const char *stroke) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(mejiro_commands_zmk); i++) {
        ...
        case MJ_CMD_KEY: {
            tap_key(mejiro_commands_zmk[i].keycodee);
            return true;
        }
        case MJ_CMD_MOD_KEY: {
...
static uint32_t keycodee_from_ascii_letter(char c) {
    if (c >= 'a' && c <= 'z') {
        c = (char)(c - 'a' + 'A');
    }
    switch (c) {
...
static void send_ascii_letters_only(const char *s) {
    for (const char *p = s; *p; p++) {
        ...
        uint32_t kc = keycodee_from_ascii_letter(c);
        if (kc != NONE) {
            tap_key(kc);
        }
    }
...
void ng_type(NGList *keys) {
    MJ_MJ_LOG_DBG(">NAGINATA NG_TYPE");

    if (keys->size == 0)
        return;

    if (keys->size == 1 && keys->elements[0] == ENTER) {
        MJ_MJ_LOG_DBG(" NAGINATA type keycodee 0x%02X", ENTER);
        return;
    }
            ...
            if (ngdickana[i].kana[0] != NONE) {
                for (int k = 0; k < 6; k++) {
                    if (ngdickana[i].kana[k] == NONE)
                        break;
                    MJ_MJ_LOG_DBG(" NAGINATA type keycodee 0x%02X", ngdickana[i].kana[k]);
                    tap_key(ngdickana[i].kana[k]);
}
            } else {
                ngdickana[i].func();
            }
...
bool naginata_press(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    MJ_MJ_LOG_DBG(">NAGINATA PRESS");

    uint32_t keycodee = binding->param1;
    ...
    case SQT:        
        n_pressed_keys++;
        pressed_keys |= ng_key[keycodee - A]; // キーの重ね合わせ
        chord_keys |= ng_key[keycodee - A];   // chord union for this stroke

        if (keycodee == SPACE || keycodee == ENTER) {
            NGList a;
            initializeList(&a);
            addToList(&a, keycodee);
            ...
            if (nginput.size > 0) {
                ...
                copyList(&a, &b);
                addToList(&b, keycodee);
            }

            // 前のキーとの同時押しの可能性があるなら前に足す
            // 同じキー連打を除外
            if (nginput.size > 0 && a.elements[a.size - 1] != keycodee &&
        ...
        for (int i = 0; i < 10; i++) {
            ...
            int c = includeList(&rskc, keycodee);
            uint32_t brs = 0UL;
...
bool naginata_release(struct zmk_behavior_binding *binding,
    ...
    MJ_MJ_LOG_DBG(">NAGINATA RELEASE");
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
                                     struct zmk_behavior_binding_event event) {
    MJ_MJ_LOG_DBG("position %d keycodee 0x%02X", event.position, binding->param1);
