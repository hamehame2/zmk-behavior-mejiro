...
static bool mejiro_contains_hash(const char *s) {
    return s && strchr(s, '#') != NULL;
}

static void mejiro_remove_hash(const char *src, char *dst, size_t dst_sz) {
    if (!dst || dst_sz == 0) {
        return;
    }
    if (!src) {
...
static bool handle_mejiro_command(const char *stroke) {
    ...
    bool has_hash = mejiro_contains_hash(stroke);
    char hashless[64] = {0};

    for (size_t pass = 0; pass < (has_hash ? 2 : 1); pass++) {
        const char *lookup = stroke;
        bool doubled = false;

        if (pass == 1) {
            mejiro_remove_hash(stroke, hashless, sizeof(hashless));
            if (hashless[0] == ' ') {
                continue;
            }
            lookup = hashless;
        ...
        for (size_t i = 0; i < ARRAY_SIZE(mejiro_commands_zmk); i++) {
            if (strcmp(mejiro_commands_zmk[i].stroke, lookup) != 0) {
                continue;
            }

            switch (mejiro_commands_zmk[i].kind) {
            case MJ_CMD_REPEAT:
                if (g_mejiro_last_output[0] != ' ') {
...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    bool has_hash = mejiro_contains_hash(mejiro_id);
    char effective_id[64] = {0};
    const char *parse_id = mejiro_id;
    if (has_hash) {
        mejiro_remove_hash(mejiro_id, effective_id, sizeof(effective_id));
        if (effective_id[0] != ' ') {
            parse_id = effective_id;
        }
    }

    char left[32] = {0};
    char right[32] = {0};
    parse_mejiro_id(parse_id, left, right);

    // 削除操作（-U、-AU）の場合は持ち越しの「っ」をクリア
    if (strcmp(parse_id, "-U") == 0 || strcmp(parse_id, "-AU") == 0) {
        pending_tsu = false;
    }
    ...
    if (strlen(result.output) > 0 && !is_right_only) {
        ...
        if (has_hash) {
            char once[128];
            strncpy(once, result.output, sizeof(once) - 1);
            once[sizeof(once) - 1] = ' ';
            size_t once_len = strlen(once);
            size_t avail = sizeof(result.output) - 1 - strlen(result.output);
            if (once_len > 0 && avail >= once_len) {
                strcat(result.output, once);
