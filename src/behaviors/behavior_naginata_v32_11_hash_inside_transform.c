...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    bool has_hash = false;
    char transform_id[64] = {0};
    if (mejiro_id != NULL) {
        size_t j = 0;
        for (size_t i = 0; mejiro_id[i] != ' ' && j + 1 < sizeof(transform_id); i++) {
            if (mejiro_id[i] == '#') {
                has_hash = true;
                continue;
            }
            transform_id[j++] = mejiro_id[i];
        }
        transform_id[j] = ' ';
    }
    const char *effective_id = has_hash ? transform_id : mejiro_id;

    char left[32] = {0};
    char right[32] = {0};
    parse_mejiro_id(effective_id, left, right);

    // 削除操作（-U、-AU）の場合は持ち越しの「っ」をクリア
    if (strcmp(effective_id, "-U") == 0 || strcmp(effective_id, "-AU") == 0) {
        pending_tsu = false;
    }
    ...
    if (strlen(result.output) > 0 && !is_right_only) {
        ...
        if (has_hash) {
            ...
            strncat(repeated, result.output, sizeof(repeated) - strlen(repeated) - 1);
            strncpy(result.output, repeated, sizeof(result.output) - 1);
            result.output[sizeof(result.output) - 1] = ' ';
