...
static void build_mejiro_id(uint32_t chord, char *out, size_t out_sz) {
     ...
     *
     * Safety rule:
     *   when a proxy is active, overlapping physical keys on the same side are
     *   suppressed so we do not emit duplicated letters like SS*, nnkk, etc.
     *   This keeps the current physical layout while making accidental overlap safe.
     */
    const bool proxy_s_star    = (chord & B_SQT) != 0;
    const bool proxy_ntk_left  = (chord & B_Z) != 0;
    const bool proxy_nk_left   = (chord & B_X) != 0;
    const bool proxy_ntk_right = (chord & B_DOT) != 0;
    const bool proxy_nk_right  = (chord & B_SLASH) != 0;

    const bool suppress_left_n = proxy_ntk_left || proxy_nk_left;
    const bool suppress_left_t = proxy_ntk_left;
    const bool suppress_left_k = proxy_ntk_left || proxy_nk_left;

    const bool suppress_right_s    = proxy_s_star;
    const bool suppress_right_n    = proxy_ntk_right || proxy_nk_right;
    const bool suppress_right_t    = proxy_ntk_right;
    const bool suppress_right_k    = proxy_ntk_right || proxy_nk_right;
    const bool suppress_right_star = proxy_s_star;
    ...
    if ((chord & B_C) && !suppress_left_n) APP('n');   // C左n
    if ((chord & B_V) && !suppress_left_t) APP('t');   // V左t
    if ((chord & B_B) && !suppress_left_k) APP('k');   // B左k
    if (chord & B_Q) APP('#');   // Q左#

    if (proxy_ntk_left) { APP('n'); APP('t'); APP('k'); }
    if (proxy_nk_left)  { APP('n'); APP('k'); }

    APP('-');

    // --- Right: consonants STKN (SEMI,O,L,K) + vowels YIAU (I,U,J,(H|Y)) + particles ntk (COMMA,M,N) + optional '*'
    if ((chord & B_SEMI) && !suppress_right_s) APP('S'); // SEMI右S
    ...
    /* proxy S* must inject S here, in the right consonant block */
    if (proxy_s_star) APP('S');

    if ((chord & B_COMMA) && !suppress_right_n) APP('n'); // COMMA右n
    if ((chord & B_M) && !suppress_right_t) APP('t');     // M右t
    if ((chord & B_N) && !suppress_right_k) APP('k');     // N右k

    if (proxy_ntk_right) { APP('n'); APP('t'); APP('k'); }
    if (proxy_nk_right)  { APP('n'); APP('k'); }

    if ((chord & B_P) && !suppress_right_star) APP('*');  // P右*
    if (proxy_s_star) APP('*');
