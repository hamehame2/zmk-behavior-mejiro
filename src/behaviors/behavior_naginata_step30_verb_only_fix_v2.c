...
mejiro_result_t_zmk mejiro_transform_zmk(const char *mejiro_id) {
    ...
    if (has_asterisk) {
        ...
        // ただし * 経路ではこの時点ではまだ母音補完前なので、
        // 動詞照合用にだけ補完した値を別変数で作る。
        {
            char verb_l_vowel[16] = {0};
            char verb_r_vowel[16] = {0};
            strcpy(verb_l_vowel, l_vowel);
            strcpy(verb_r_vowel, r_vowel);

            if (strlen(verb_l_vowel) == 0 && strlen(l_conso) > 0 && strcmp(l_conso, "STN") != 0) {
                strcpy(verb_l_vowel, last_vowel_stroke);
            }
            if (strlen(verb_r_vowel) == 0 && strlen(r_conso) > 0 && strcmp(r_conso, "STN") != 0) {
                if (strlen(verb_l_vowel) > 0) {
                    strcpy(verb_r_vowel, verb_l_vowel);
                } else {
                    strcpy(verb_r_vowel, last_vowel_stroke);
                }
            }

            char exact_verb_kana[128] = {0};
            if (mejiro_conjugate_exact_dict_verb(
                    l_conso, verb_l_vowel, l_particle_str,
                    r_conso, verb_r_vowel, r_particle_str,
        ...
        if (strlen(l_conso) > 0 || strlen(verb_l_vowel) > 0) {
            convert_to_kana(l_conso, verb_l_vowel, "", false, left_kana_temp);
        }
        if (strlen(r_conso) > 0 || strlen(verb_r_vowel) > 0) {
            convert_to_kana(r_conso, verb_r_vowel, "", false, right_kana_temp);
        }
