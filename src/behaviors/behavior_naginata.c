 ...
 * Restored from uploaded QMK mejiro_verb.c / mejiro_verb.h
 */

// ------------------------------------------------------------
// Verb dictionary types (from QMK mejiro_verb.h)
// ------------------------------------------------------------
typedef enum {
    VERB_TYPE_GODAN,
    VERB_TYPE_KAMI,
...
// QMK と同じ活用順
// CONJ_NAI=0, CONJ_SHIEKI=1, CONJ_UKEMI=2, CONJ_MASU=3, CONJ_JISHO=4,
// CONJ_TE_TA=5, CONJ_IKOU=6, CONJ_KATEI=7, CONJ_KANOU=8, CONJ_MEIREI=9
typedef enum {
    CONJ_NAI = 0,
    CONJ_SHIEKI = 1,
    CONJ_UKEMI = 2,
...
static bool mejiro_conjugate_exact_dict_verb(const char *left_conso, const char *left_vowel,
    ...
    verb_result_t result = mejiro_verb_conjugate(left_conso, left_vowel,
                                                 left_particle,
                                                 right_conso, right_vowel,
                                                 right_particle,
                                                 "", "");
    if (!result.success) {
        return false;
    ...
    out[0] = '\0';
    strncat(out, result.output, out_size - 1);
    out[out_size - 1] = '\0';
    return true;
}
...
static void press_key(uint32_t keycode);
static void release_key(uint32_t keycode);
