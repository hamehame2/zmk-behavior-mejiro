...
// Conjugation forms (aligned to QMK mejiro_verb.h)
typedef enum {
    CONJ_NAI = 0,    // ない形
    CONJ_SHIEKI = 1, // 使役形
    CONJ_UKEMI = 2,  // 受身形
...
static bool mejiro_conjugate_exact_dict_verb(const char *left_conso, const char *left_vowel,
    ...
    if (strlen(left_conso) > 0 || strlen(left_vowel) > 0) {
        convert_to_kana(left_conso, left_vowel, "", false, left_kana_temp);
    }
    if (strlen(right_conso) > 0 || strlen(right_vowel) > 0) {
        convert_to_kana(right_conso, right_vowel, "", false, right_kana_temp);
    }

    verb_result_t vr = mejiro_verb_conjugate(
        left_conso, left_vowel, left_particle,
        right_conso, right_vowel, right_particle,
    ...
    strncpy(out, vr.output, out_size - 1);
    out[out_size - 1] = '\0';
    return true;
}

static verb_result_t mejiro_verb_conjugate(const char *left_conso, const char *left_vowel,
                                    const char *left_particle,
                                    const char *right_conso, const char *right_vowel,
                                    const char *right_particle,
