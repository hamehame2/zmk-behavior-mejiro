...
/* Flush a keyboard report to host.
 * NOTE: In this ZMK tree, zmk_endpoints_send_report() requires a usage page argument.
...
/* Send a key tap as two distinct HID reports (press then release).
 * Without a separation, some hosts/endpoints can miss the release and start OS key-repeat.
 */
zmk_hid_keyboard_press(keycode);
ng_send_report();
/* Ensure the host can observe the intermediate (pressed) report. */
