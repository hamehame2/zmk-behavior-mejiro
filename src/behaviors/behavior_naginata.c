...
#else
/*
 * Some ZMK trees do not expose these helpers via a public header.
 * Declare prototypes so we can still build without relying on implicit-int.
 */
int zmk_hid_keyboard_press(zmk_key_t keycode);
int zmk_hid_keyboard_release(zmk_key_t keycode);
#endif
...
static zmk_key_t ng_tapq[NG_TAPQ_CAP];
...
static void tap_key(zmk_key_t keycode) {
    ng_tap_key(keycode);
}
...
static inline void press_key(zmk_key_t keycode) {
    zmk_hid_keyboard_press(keycode);
}

static inline void release_key(zmk_key_t keycode) {
    zmk_hid_keyboard_release(keycode);
}

static inline void mod_tap(uint32_t mod_keycodee, zmk_key_t keycode) {
...
/* Mod keycodees (explicit HID usages, avoids relying on keycode aliases) */
...
void ng_type(NGList *keys) {
    ...
    if (keys->size == 1 && keys->elements[0] == ENTER) {
        MJ_MJ_LOG_DBG(" NAGINATA type keycode 0x%02X", ENTER);
        return;
    }
            ...
            if (ngdickana[i].kana[0] != NONE) {
                for (int k = 0; k < 6; k++) {
                    if (ngdickana[i].kana[k] == NONE)
                        break;
                    MJ_MJ_LOG_DBG(" NAGINATA type keycode 0x%02X", ngdickana[i].kana[k]);
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
                                     struct zmk_behavior_binding_event event) {
    MJ_MJ_LOG_DBG("position %d keycode 0x%02X", event.position, binding->param1);
