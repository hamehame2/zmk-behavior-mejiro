...
static void mejiro_tx_step(struct k_work *work) {
    (void)work;
    ...
    if (g_mejiro_tx_pos >= g_mejiro_tx_len || g_mejiro_tx_buf[g_mejiro_tx_pos] == "\0") {
        g_mejiro_tx_busy = false;
        g_mejiro_tx_len = 0;
        g_mejiro_tx_pos = 0;
        g_mejiro_tx_buf[0] = "\0";
        return;
    ...
    char one[2];
    one[0] = g_mejiro_tx_buf[g_mejiro_tx_pos++];
    one[1] = "\0";
    send_ascii_letters_only(one);

    (void)k_work_schedule(&g_mejiro_tx_work, K_MSEC(MEJIRO_TX_INTERVAL_MS));
}
...
static void send_string(const char *s) {
    ...
    if (!g_mejiro_tx_inited) {
        k_work_init_delayable(&g_mejiro_tx_work, mejiro_tx_step);
        g_mejiro_tx_inited = true;
    }

    // Copy into static buffer (bounded)
    size_t n = strnlen(s, sizeof(g_mejiro_tx_buf) - 1);
    memcpy(g_mejiro_tx_buf, s, n);
    g_mejiro_tx_buf[n] = "\0";
    g_mejiro_tx_len = n;
    g_mejiro_tx_pos = 0;

    // Start (or restart) sending
    g_mejiro_tx_busy = true;
    (void)k_work_schedule(&g_mejiro_tx_work, K_NO_WAIT);
}
