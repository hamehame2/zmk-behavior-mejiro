...
bool naginata_press(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    ...
    case SQT: {
        n_pressed_keys++;
        pressed_keys |= ng_key[keycode - A]; // 押下中キー集合
        chord_keys |= ng_key[keycode - A];   // このストロークのchord集合（release確定用）

        /* ---- Safe building of nginput (never crash / never block) ---- */

        if (keycode == SPACE || keycode == ENTER) {
            NGList a;
            initializeList(&a);
            (void)addToList(&a, keycode);

            if (!addToListArray(&nginput, &a)) {
                /* Buffer overflow or API mismatch should NEVER hard-fault */
            ...
            /* Always initialize locals: never read uninitialized memory */
            NGList a;
            NGList b;
            initializeList(&a);
            initializeList(&b);

            /* b = (last chord) + keycode, or just keycode if this is the first key */
            if (nginput.size > 0) {
                copyList(&(nginput.elements[nginput.size - 1]), &a);
                copyList(&a, &b);
            }
            (void)addToList(&b, keycode);

            /* If we can merge with previous chord (true chord), replace last entry */
            if (nginput.size > 0 && a.size > 0 &&
                a.elements[a.size - 1] != keycode &&
                number_of_candidates(&b) > 0) {

                if (!removeFromListArrayAt(&nginput, nginput.size - 1) ||
                    !addToListArray(&nginput, &b)) {
                    /* Never allow corrupted list state to persist */
                    initializeListArray(&nginput);
                    pressed_keys = 0UL;
                    chord_keys = 0UL;
                    n_pressed_keys = 0;
                ...
                /* Not a chord: append single-key list */
                NGList e;
                initializeList(&e);
                (void)addToList(&e, keycode);
        ...
        /* If nginput is empty for any reason, stop here safely */
        if (nginput.size <= 0 || nginput.elements[0].size <= 0) {
            break;
        }

        /* ---- Continuous shift handling (must also be safe) ---- */
        static const uint32_t rs[10][2] = {
            {D, F}, {C, V}, {J, K}, {M, COMMA}, {SPACE, 0},
        ...
        for (int i = 0; i < 10; i++) {
            NGList rskc;
            initializeList(&rskc);
            (void)addToList(&rskc, rs[i][0]);
            if (rs[i][1] > 0) {
                (void)addToList(&rskc, rs[i][1]);
            }

            const int c = includeList(&rskc, keycode);
            uint32_t brs = 0UL;
            ...
            for (int j = 0; j < l.size; j++) {
                (void)addToList(&rskc, l.elements[j]);
            }

            if (c < 0 &&
                ((brs & pressed_keys) == brs) &&
                (keyset & brs) != brs &&
        ...
        /* Mejiro finalize-on-release: do NOT commit on press. */
        break;
    }
    default:
        break;
...
bool naginata_release(struct zmk_behavior_binding *binding,
    ...
    case SQT:
        ...
        if (pressed_keys == 0UL) {
            ...
            /* Clear pending Naginata candidate buffer safely (never infinite loop). */
            for (int guard = 0; guard < MAX_LIST_ARRAY_SIZE && nginput.size > 0; guard++) {
                if (!removeFromListArrayAt(&nginput, 0)) {
                    /* If list state is corrupted, reset hard but safely */
                    initializeListArray(&nginput);
                    break;
                }
            }
            if (nginput.size > 0) {
                /* If still non-empty after guard, force reset */
                initializeListArray(&nginput);
            }
        } else {
            /* Still holding: do not commit */
        }
        break;
    default:
        break;
