...
// Mejiro-specific pending state (must be defined before any reset helpers)
static bool pending_tsu = false;
  // current chord (union of pressed keys for this stroke)

// ---------- Safety reset (must NEVER block / crash) ----------
static inline void naginata_clear_state(void) {
    initializeListArray(&nginput);
    pressed_keys = 0UL;
    chord_keys = 0UL;
    n_pressed_keys = 0;
    // Mejiro-specific pending state
    pending_tsu = false;
}

static inline bool safe_add_to_list(NGList *list, uint32_t element) {
    if (!addToList(list, element)) {
        naginata_clear_state();
        return false;
    }
    return true;
}

static inline bool safe_add_to_list_array(NGListArray *list, NGList *element) {
    if (!addToListArray(list, element)) {
        naginata_clear_state();
        return false;
    }
    return true;
...
bool naginata_press(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    ...
    case SQT:        
        ...
        if (keycode == SPACE || keycode == ENTER) {
            NGList a;
            initializeList(&a);
            if (!safe_add_to_list(&a, keycode)) { return ZMK_BEHAVIOR_OPAQUE; }
            if (!safe_add_to_list_array(&nginput, &a)) { return ZMK_BEHAVIOR_OPAQUE; }
        } else {
            NGList a;
            NGList b;
            if (nginput.size > 0) {
            ...
            if (nginput.size > 0 && a.elements[a.size - 1] != keycode &&
                ...
                if (!safe_add_to_list_array(&nginput, &b)) { return ZMK_BEHAVIOR_OPAQUE; }
                // 前のキーと同時押しはない
            } else {
                // 連続シフトではない
                NGList e;
                initializeList(&e);
                if (!safe_add_to_list(&e, keycode)) { return ZMK_BEHAVIOR_OPAQUE; }
                if (!safe_add_to_list_array(&nginput, &e)) { return ZMK_BEHAVIOR_OPAQUE; }
            }
        }
