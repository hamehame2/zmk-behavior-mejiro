...
static bool ng_committed = false;
...
static void send_ascii_letters_only(const char *s) {
    ...
    if (zmk_hid_keyboard_clear) { zmk_hid_keyboard_clear(); }
}
...
bool naginata_press(struct zmk_behavior_binding *binding, struct zmk_behavior_binding_event event) {
    ...
    case SEMI:
    case SQT:        
        if (n_pressed_keys == 0) { ng_committed = false; }
        n_pressed_keys++;
...
if (pressed_keys == 0UL) {
    if (ng_committed) { return ZMK_BEHAVIOR_OPAQUE; }
    ng_committed = true;
