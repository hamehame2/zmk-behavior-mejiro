...
static bool execute_mejiro_command_exact(const char *stroke, bool doubled) {
...
static bool handle_mejiro_command(const char *stroke) {
    return execute_mejiro_command_exact(stroke, false);
}
...
static void process_mejiro_stroke(const char *stroke) {
    if (!stroke || stroke[0] == '\0') {
        return;
    }

    /* 1) original exact command path, including # commands explicitly present */
    if (handle_mejiro_command(stroke)) {
        return;
    }

    /* 2) no # => preserve original single-file behavior */
    if (!mejiro_contains_hash(stroke)) {
        process_mejiro_stroke(stroke);
        return;
    }

    /* 3) hashless command retry (QMK's second command-table pass) */
    char hashless[64] = {0};
    ...
    if (execute_mejiro_command_exact(hashless, true)) {
        return;
    }

    /* 4) hashless normal transform: keep original transform path untouched */
    send_mejiro_output(hashless);
    send_mejiro_output(hashless);
}
