...
static int behavior_naginata_init(const struct device *dev) {
    ...
    /* Init Mejiro async sender once (do NOT re-init on every send). */
...
static void mejiro_send_work_handler(struct k_work *work) {
    ...
    // safe_mode:
    //  3: never schedule this work (see submit paths)
    //  2: run work but do nothing (isolate scheduler vs sender/queue)
    //  1: send ASCII letters/spaces only (minimal TX)
    //  0: full behavior
    if (g_mejiro_safe_mode == 2) {
        // Clear any pending buffer/queue and exit without sending.
        g_mejiro_send_pos = 0;
        g_mejiro_send_len = 0;
        return;
    }
    ...
    (void)if (g_mejiro_safe_mode != 3) k_work_schedule(&g_mejiro_send_work, K_MSEC(2));
}
...
static void mejiro_queue_send_ascii(const char *s) {
    ...
    (void)if (g_mejiro_safe_mode != 3) k_work_schedule(&g_mejiro_send_work, K_NO_WAIT);
}
