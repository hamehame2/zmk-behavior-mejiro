...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    if (has_hash) {
         ...
         */
        size_t w = 0;
        for (size_t r = 0; mejiro_id[r] != '' && w < (sizeof(id_no_hash) - 1); r++) {
            if (mejiro_id[r] != '#') {
    ...
    const majiro_result_t_zmk r = mejiro_transform_zmk(id_for_translate);

    if (!r.success || r.output == NULL || r.output[0] == '') {
    ...
    /* store doubled output for subsequent repeat */
    snprintf(g_mejiro_last_output, sizeof(g_mejiro_last_output), "%s%s", r.output, r.output);
    mejiro_history_push((uint16_t)strlen(g_mejiro_last_output));
}
