...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    const char *transform_pattern = mejiro_id;

    if (has_hash) {
        mejiro_remove_hash(mejiro_id, pattern_without_hash, sizeof(pattern_without_hash));
        if (pattern_without_hash[0] == ' ') {
            return;
        }
        transform_pattern = pattern_without_hash;
    }
