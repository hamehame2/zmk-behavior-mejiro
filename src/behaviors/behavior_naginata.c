...
#define MEJIRO_ROMA_KEY_DELAY_MS 10
#endif
...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    bool has_hash = false;
    char pattern_without_hash[64];
    const char *transform_pattern = mejiro_id;

    for (size_t i = 0; mejiro_id[i] != '\0'; i++) {
        if (mejiro_id[i] == '#') {
            has_hash = true;
            break;
        }
    ...
    if (has_hash) {
        size_t j = 0;
        for (size_t i = 0; mejiro_id[i] != '\0' && j < sizeof(pattern_without_hash) - 1; i++) {
            if (mejiro_id[i] != '#') {
                pattern_without_hash[j++] = mejiro_id[i];
            }
        }
        pattern_without_hash[j] = '\0';
        if (pattern_without_hash[0] != '\0') {
