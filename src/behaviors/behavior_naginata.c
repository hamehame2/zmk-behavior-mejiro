...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    /* Keep v32_2 command path unchanged; only normal transform gets # repeat. */
    ...
    if (has_hash) {
        size_t j = 0;
        for (size_t i = 0; mejiro_id[i] != ' ' && j < sizeof(pattern_without_hash) - 1; i++) {
            if (mejiro_id[i] != '#') {
                pattern_without_hash[j++] = mejiro_id[i];
            }
        }
        pattern_without_hash[j] = ' ';
