...
// Conjugation forms (QMK mejiro_verb.h order)
typedef enum {
    CONJ_NAI,     // ない形
    CONJ_SHIEKI,  // 使役形
    CONJ_UKEMI,   // 受身形
...
static verb_result_t mejiro_verb_conjugate(const char *left_conso, const char *left_vowel,
                                    const char *left_particle, const char *right_conso,
                                    const char *right_vowel, const char *right_particle,
...
static bool mejiro_conjugate_exact_dict_verb(const char *left_conso, const char *left_vowel,
    ...
    verb_result_t vr = mejiro_verb_conjugate(left_conso, left_vowel, left_particle,
