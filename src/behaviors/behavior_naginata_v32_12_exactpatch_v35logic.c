...
static uint32_t keycode_from_ascii_letterstatic uint32_t keycode_from_ascii_letter(char c) {
    if (c >= 'a' && c <= 'z') {
        c = (char)(c - 'a' + 'A');
    }
    switch (c) {
...
static void send_mejiro_output(const char *mejiro_id) {
    ...
    if (has_hash) {
        mejiro_remove_hash(mejiro_id, transform_pattern, sizeof(transform_pattern));
    } else {
        strncpy(transform_pattern, mejiro_id, sizeof(transform_pattern) - 1);
        transform_pattern[sizeof(transform_pattern) - 1] = '\0';
    }
    ...
    mejiro_result_t_zmk result = mejiro_transform_zmk(transform_pattern);
    if (!result.success) {
        return;
    }
