...
#else
#define MJ_MJ_LOG_DBG(...) MJ_LOG_DBG(__VA_ARGS__)
#endif
...
void ng_type(NGList *keys) {
    MJ_LOG_DBG(">NAGINATA NG_TYPE");

    if (keys->size == 0)
        return;

    if (keys->size == 1 && keys->elements[0] == ENTER) {
        MJ_LOG_DBG(" NAGINATA type keycode 0x%02X", ENTER);
            ...
            if (ngdickana[i].kana[0] != NONE) {
                for (int k = 0; k < 6; k++) {
                    if (ngdickana[i].kana[k] == NONE)
                        break;
                    MJ_LOG_DBG(" NAGINATA type keycode 0x%02X", ngdickana[i].kana[k]);
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
                                     struct zmk_behavior_binding_event event) {
    MJ_LOG_DBG("position %d keycode 0x%02X", event.position, binding->param1);
